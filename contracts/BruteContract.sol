...
contract BruteGame is ERC721 {
    ...
    function calculateXPForNextLevel(uint256 currentLevel) internal pure returns (uint256) {
    ...
    function updateLevel(uint256 bruteId) public {
        ...
        uint256 requiredXP = calculateXPForNextLevel(brute.level + 1);
        if (brute.experience >= requiredXP) {
            brute.level++;
        }
    }

    // Function to handle XP gained from a combat
    ...
    function resolveCombatAndDistributeXP(uint256 bruteId1, uint256 bruteId2) public returns (uint256 winnerId) {
        ...
        Brute memory brute1 = brutes[bruteId1];
        Brute memory brute2 = brutes[bruteId2];
        ...
        require(brute2.owner != msg.sender, "You own both brute");
        ...
        brutes[bruteId1].health = base_health;
        brutes[bruteId2].health = base_health;


    }

    // Function to update stats of brute after assigning ability
    ...
    function assignAbilityToBrute(uint256 bruteId, uint256 abilityId) external {
        ...
        require(brute.level >= ability.requiredLevel, "Ability does not exist");
    ...
    // Function to check if a brute has a specific ability
